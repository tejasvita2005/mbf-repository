...
export function ExerciseSection() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          setCompleted(new Set(data.map((d) => d.exercise_id)));
        }
  ...
  if (loading) {
    return (
      <div className="glass rounded-2xl border border-white/7 p-6 text-white">
        Loading exercises...
      </div>
    );
  }
        ...
        {filtered.map((exercise) => {
          ...
          return (
            <div
              ...
              className={`flex items-center gap-4 px-5 py-4 transition-all duration-200 hover:bg-white/3 cursor-pointer ${
                isDone ? 'opacity-60' : ''
              }`}
            >
              {/* Checkbox */}
